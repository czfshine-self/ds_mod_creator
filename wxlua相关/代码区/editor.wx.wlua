...
-- wxWindow variables
frame22            = nil    -- wxframe the main top level window
...
-- Create the wxframe
-- ----------------------------------------------------------------------------
frame22= wx.wxFrame(wx.NULL, wx.wxID_ANY, "wxLua")

statusBar = frame22:CreateStatusBar( 4 )
local status_txt_width = statusBar:GetTextExtent("OVRW")
frame22:SetStatusWidths({-1, status_txt_width, status_txt_width, status_txt_width*5})
frame22:SetStatusText("Welcome to wxLua")

toolBar = frame22:CreateToolBar(wx.wxNO_BORDER + wx.wxTB_FLAT + wx.wxTB_DOCKABLE)
...
-- ----------------------------------------------------------------------------
-- Add the child windows to the frame22

splitter = wx.wxSplitterWindow(frame22, wx.wxID_ANY,
...
function ConfigRestoreframe22Position(window, windowName)
...
function ConfigSaveframe22Position(window, windowName)
    local path = config:GetPath()
    config:SetPath("/"..windowName)
...
-- ----------------------------------------------------------------------------
-- Update the statusbar text of the frame22 using the given editor.
--  Only update if the text has changed.
statusTextTable = { "OVR?", "R/O?", "Cursor Pos" }

function UpdateStatusText(editor)
    local texts = { "", "", "" }
    if frame22 and editor then
    ...
    if frame22 then
        for n = 1, 3 do
            if (texts[n] ~= statusTextTable[n]) then
                frame22:SetStatusText(texts[n], n)
        ...
        if filePath and (string.len(filePath) > 0) and oldModTime and oldModTime:IsValid() then
            ...
            elseif modTime:IsValid() and oldModTime:IsEarlierThan(modTime) then
                local ret = wx.wxMessageBox(fileName.." has been modified on disk.\nDo you want to reload it?",
                                            "wxLua Message",
                                            wx.wxYES_NO + wx.wxCENTRE, frame22)
                    ...
                    if (pos - start_pos > 0) and (start_pos > 2) then
                        ...
                        if range == "wx." then
                            local commandEvent = wx.wxCommandEvent(wx.wxEVT_COMMAND_MENU_SELECTED,
                                                                   ID_AUTOCOMPLETE)
                            wx.wxPostEvent(frame22, commandEvent)
                        end
                    end
                end
            end)
...
function CreateWatchWindow()
    local width = 180
    watchWindow = wx.wxframe(frame22, wx.wxID_ANY, "wxLua Watch Window",
    ...
    ConfigRestoreframe22Position(watchWindow, "WatchWindow")
    ...
    watchWindow:Connect( wx.wxEVT_CLOSE_WINDOW,
            function (event)
                ConfigSaveframe22Position(watchWindow, "WatchWindow")
                watchWindow = nil
                watchListCtrl = nil
                event:Skip()
            end)
...
function UpdateUIMenuItems()
    if frame22 and frame22:GetMenuBar() then
        for n = 0, frame22:GetMenuBar():GetMenuCount()-1 do
            frame22:GetMenuBar():GetMenu(n):UpdateUI()
        end
    end
end
...
frame22:Connect(ID_NEW, wx.wxEVT_COMMAND_MENU_SELECTED, NewFile)
...
function OpenFile(event)
    local fileDialog = wx.wxFileDialog(frame22, "Open file",
                                       "",
                                       "",
...
frame22:Connect(ID_OPEN, wx.wxEVT_COMMAND_MENU_SELECTED, OpenFile)
...
frame22:Connect(ID_SAVE, wx.wxEVT_COMMAND_MENU_SELECTED,
        function (event)
...
frame22:Connect(ID_SAVE, wx.wxEVT_UPDATE_UI,
        function (event)
            local editor = GetEditor()
            if editor then
                local id = editor:GetId()
...
function SaveFileAs(editor)
    ...
    local fileDialog = wx.wxFileDialog(frame22, "Save file as",
                                       fn:GetPath(),
                                       fn:GetFullName(),
                                       "Lua files (*.lua)|*.lua|Text files (*.txt)|*.txt|All files (*)|*",
                                       wx.wxSAVE)
...
frame22:Connect(ID_SAVEAS, wx.wxEVT_COMMAND_MENU_SELECTED,
        function (event)
            local editor = GetEditor()
            SaveFileAs(editor)
        end)
frame22:Connect(ID_SAVEAS, wx.wxEVT_UPDATE_UI,
        function (event)
            local editor = GetEditor()
...
frame22:Connect(ID_SAVEALL, wx.wxEVT_COMMAND_MENU_SELECTED,
        function (event)
            SaveAll()
        end)

frame22:Connect(ID_SAVEALL, wx.wxEVT_UPDATE_UI,
        function (event)
...
function SaveModifiedDialog(editor, allow_cancel)
    ...
    if document.isModified then
        ...
        local dialog = wx.wxMessageDialog(frame22, message,
                                          "Save Changes?",
                                          dlg_styles)
...
frame22:Connect(ID_CLOSE, wx.wxEVT_COMMAND_MENU_SELECTED,
        function (event)
            local editor = GetEditor()
            local id     = editor:GetId()
...
frame22:Connect(ID_CLOSE, wx.wxEVT_UPDATE_UI,
        function (event)
...
frame22:Connect( ID_EXIT, wx.wxEVT_COMMAND_MENU_SELECTED,
        function (event)
            if not SaveOnExit(true) then return end
            frame22:Close() -- will handle wxEVT_CLOSE_WINDOW
            CloseWatchWindow()
        end)
...
frame22:Connect(ID_CUT, wx.wxEVT_COMMAND_MENU_SELECTED, OnEditMenu)
frame22:Connect(ID_CUT, wx.wxEVT_UPDATE_UI, OnUpdateUIEditMenu)

frame22:Connect(ID_COPY, wx.wxEVT_COMMAND_MENU_SELECTED, OnEditMenu)
frame22:Connect(ID_COPY, wx.wxEVT_UPDATE_UI, OnUpdateUIEditMenu)

frame22:Connect(ID_PASTE, wx.wxEVT_COMMAND_MENU_SELECTED, OnEditMenu)
frame22:Connect(ID_PASTE, wx.wxEVT_UPDATE_UI,
        function (event)
            local editor = GetEditor()
...
frame22:Connect(ID_SELECTALL, wx.wxEVT_COMMAND_MENU_SELECTED, OnEditMenu)
frame22:Connect(ID_SELECTALL, wx.wxEVT_UPDATE_UI, OnUpdateUIEditMenu)

frame22:Connect(ID_UNDO, wx.wxEVT_COMMAND_MENU_SELECTED, OnEditMenu)
frame22:Connect(ID_UNDO, wx.wxEVT_UPDATE_UI,
        function (event)
            local editor = GetEditor()
            event:Enable(editor and editor:CanUndo())
        end)

frame22:Connect(ID_REDO, wx.wxEVT_COMMAND_MENU_SELECTED, OnEditMenu)
frame22:Connect(ID_REDO, wx.wxEVT_UPDATE_UI,
        function (event)
            local editor = GetEditor()
            event:Enable(editor and editor:CanRedo())
        end)

frame22:Connect(ID_AUTOCOMPLETE, wx.wxEVT_COMMAND_MENU_SELECTED,
        function (event)
            local editor = GetEditor()
            if (editor == nil) then return end
            local pos = editor:GetCurrentPos()
...
frame22:Connect(ID_AUTOCOMPLETE, wx.wxEVT_UPDATE_UI, OnUpdateUIEditMenu)

frame22:Connect(ID_AUTOCOMPLETE_ENABLE, wx.wxEVT_COMMAND_MENU_SELECTED,
        function (event)
            autoCompleteEnable = event:IsChecked()
        end)

frame22:Connect(ID_COMMENT, wx.wxEVT_COMMAND_MENU_SELECTED,
        function (event)
            local editor = GetEditor()
            local buf = {}
...
frame22:Connect(ID_COMMENT, wx.wxEVT_UPDATE_UI, OnUpdateUIEditMenu)
...
frame22:Connect(ID_FOLD, wx.wxEVT_COMMAND_MENU_SELECTED,
        function (event)
            FoldSome()
        end)
frame22:Connect(ID_FOLD, wx.wxEVT_UPDATE_UI, OnUpdateUIEditMenu)
...
function findReplace:FindString(reverse)
    if findReplace:HasText() then
        ...
        if posFind == -1 then
            ...
            frame22:SetStatusText("Find text not found.")
        else
            findReplace.foundString = true
...
function CreateFindReplaceDialog(replace)
    ...
    local findDialog = wx.wxDialog(frame22, wx.wxID_ANY, "Find",  wx.wxDefaultPosition, wx.wxDefaultSize)
...
frame22:Connect(ID_FIND, wx.wxEVT_COMMAND_MENU_SELECTED,
        function (event)
            findReplace:GetSelectedString()
            findReplace:Show(false)
        end)
frame22:Connect(ID_FIND, wx.wxEVT_UPDATE_UI, OnUpdateUIEditMenu)

frame22:Connect(ID_REPLACE, wx.wxEVT_COMMAND_MENU_SELECTED,
        function (event)
            findReplace:GetSelectedString()
            findReplace:Show(true)
        end)
frame22:Connect(ID_REPLACE, wx.wxEVT_UPDATE_UI, OnUpdateUIEditMenu)

frame22:Connect(ID_FINDNEXT, wx.wxEVT_COMMAND_MENU_SELECTED, function (event) findReplace:FindString() end)
frame22:Connect(ID_FINDNEXT, wx.wxEVT_UPDATE_UI, function (event) findReplace:HasText() end)

frame22:Connect(ID_FINDPREV, wx.wxEVT_COMMAND_MENU_SELECTED, function (event) findReplace:FindString(true) end)
frame22:Connect(ID_FINDPREV, wx.wxEVT_UPDATE_UI, function (event) findReplace:HasText() end)

-------------------- Find replace end

frame22:Connect(ID_GOTOLINE, wx.wxEVT_COMMAND_MENU_SELECTED,
        function (event)
            local editor = GetEditor()
...
frame22:Connect(ID_GOTOLINE, wx.wxEVT_UPDATE_UI, OnUpdateUIEditMenu)

frame22:Connect(ID_SORT, wx.wxEVT_COMMAND_MENU_SELECTED,
        function (event)
            local editor = GetEditor()
            local buf = {}
            for line in string.gmatch(editor:GetSelectedText()..'\n', "(.-)\r?\n") do
...
frame22:Connect(ID_SORT, wx.wxEVT_UPDATE_UI, OnUpdateUIEditMenu)
...
function DisplayOutput(message, dont_add_marker)
    if splitter:IsSplit() == false then
        local w, h = frame22:GetClientSizeWH()
...
frame22:Connect(ID_TOGGLEBREAKPOINT, wx.wxEVT_COMMAND_MENU_SELECTED,
        function (event)
            local editor = GetEditor()
            local line = editor:LineFromPosition(editor:GetCurrentPos())
            ToggleDebugMarker(editor, line)
        end)
frame22:Connect(ID_TOGGLEBREAKPOINT, wx.wxEVT_UPDATE_UI, OnUpdateUIEditMenu)
...
frame22:Connect(ID_COMPILE, wx.wxEVT_COMMAND_MENU_SELECTED,
        function (event)
            local editor = GetEditor()
            CompileProgram(editor)
        end)
frame22:Connect(ID_COMPILE, wx.wxEVT_UPDATE_UI, OnUpdateUIEditMenu)
    ...
    if openDocuments[id].isModified then
        ...
        if not openDocuments[id].filePath then
            local ret = wx.wxMessageBox("You must save the program before running it.\nPress cancel to abort running.",
                                         "Save file?",  wx.wxOK + wx.wxCANCEL + wx.wxCENTRE, frame22)
            if ret == wx.wxOK then
...
frame22:Connect(ID_RUN, wx.wxEVT_COMMAND_MENU_SELECTED,
        function (event)
...
frame22:Connect(ID_RUN, wx.wxEVT_UPDATE_UI,
        function (event)
            local editor = GetEditor()
            event:Enable((debuggerServer == nil) and (editor ~= nil))
        end)

frame22:Connect(ID_ATTACH_DEBUG, wx.wxEVT_COMMAND_MENU_SELECTED,
        function (event)
            local ok = false
...
frame22:Connect(ID_ATTACH_DEBUG, wx.wxEVT_UPDATE_UI,
        function (event)
            local editor = GetEditor()
            event:Enable((debuggerServer == nil) and (editor ~= nil))
        end)
        ...
        function (event)
            ...
            if not DebuggerIgnoreFile(eventFileName) then
                if not fileFound then
                    local fileDialog = wx.wxFileDialog(frame22,
...
frame22:Connect(ID_START_DEBUG, wx.wxEVT_COMMAND_MENU_SELECTED,
        function (event)
            local editor = GetEditor()
...
frame22:Connect(ID_START_DEBUG, wx.wxEVT_UPDATE_UI,
        function (event)
            local editor = GetEditor()
            event:Enable((debuggerServer == nil) and (editor ~= nil))
        end)

frame22:Connect(ID_STOP_DEBUG, wx.wxEVT_COMMAND_MENU_SELECTED,
        function (event)
            ClearAllCurrentLineMarkers()
...
frame22:Connect(ID_STOP_DEBUG, wx.wxEVT_UPDATE_UI,
        function (event)
            local editor = GetEditor()
            event:Enable((debuggerServer ~= nil) and (editor ~= nil))
        end)

frame22:Connect(ID_STEP, wx.wxEVT_COMMAND_MENU_SELECTED,
        function (event)
            ClearAllCurrentLineMarkers()
...
frame22:Connect(ID_STEP, wx.wxEVT_UPDATE_UI,
        function (event)
            local editor = GetEditor()
            event:Enable((debuggerServer ~= nil) and (not debuggee_running) and (editor ~= nil))
        end)

frame22:Connect(ID_STEP_OVER, wx.wxEVT_COMMAND_MENU_SELECTED,
        function (event)
            ClearAllCurrentLineMarkers()
...
frame22:Connect(ID_STEP_OVER, wx.wxEVT_UPDATE_UI,
        function (event)
            local editor = GetEditor()
            event:Enable((debuggerServer ~= nil) and (not debuggee_running) and (editor ~= nil))
        end)

frame22:Connect(ID_STEP_OUT, wx.wxEVT_COMMAND_MENU_SELECTED,
        function (event)
            ClearAllCurrentLineMarkers()
...
frame22:Connect(ID_STEP_OUT, wx.wxEVT_UPDATE_UI,
        function (event)
            event:Enable((debuggerServer ~= nil) and (not debuggee_running))
        end)

frame22:Connect(ID_CONTINUE, wx.wxEVT_COMMAND_MENU_SELECTED,
        function (event)
            ClearAllCurrentLineMarkers()
...
frame22:Connect(ID_CONTINUE, wx.wxEVT_UPDATE_UI,
        function (event)
            event:Enable((debuggerServer ~= nil) and (not debuggee_running))
        end)

frame22:Connect(ID_BREAK, wx.wxEVT_COMMAND_MENU_SELECTED,
        function (event)
            if debuggerServer then
                debuggerServer:Break()
            end
        end)
frame22:Connect(ID_BREAK, wx.wxEVT_UPDATE_UI,
        function (event)
            event:Enable((debuggerServer ~= nil) and debuggee_running)
        end)

frame22:Connect(ID_VIEWCALLSTACK, wx.wxEVT_COMMAND_MENU_SELECTED,
        function (event)
            if debuggerServer then
                debuggerServer:DisplayStackDialog(frame22)
            end
        end)
frame22:Connect(ID_VIEWCALLSTACK, wx.wxEVT_UPDATE_UI,
        function (event)
            event:Enable((debuggerServer ~= nil) and (not debuggee_running))
        end)

frame22:Connect(ID_VIEWWATCHWINDOW, wx.wxEVT_COMMAND_MENU_SELECTED,
        function (event)
            if not watchWindow then
                CreateWatchWindow()
            end
        end)
frame22:Connect(ID_VIEWWATCHWINDOW, wx.wxEVT_UPDATE_UI,
        function (event)
            event:Enable((debuggerServer ~= nil) and (not debuggee_running))
        end)

frame22:Connect(ID_SHOWHIDEWINDOW, wx.wxEVT_COMMAND_MENU_SELECTED,
        function (event)
...
frame22:Connect(ID_DEBUGGER_PORT, wx.wxEVT_COMMAND_MENU_SELECTED,
        function(event)
        end)
frame22:Connect(ID_DEBUGGER_PORT, wx.wxEVT_UPDATE_UI,
        function(event)
...
function DisplayAbout(event)
    ...
    local dlg = wx.wxDialog(frame22, wx.wxID_ANY, "About wxLua IDE")
...
frame22:Connect(ID_ABOUT, wx.wxEVT_COMMAND_MENU_SELECTED, DisplayAbout)

-- ---------------------------------------------------------------------------
-- Attach the handler for closing the frame22
...
function CloseWindow(event)
    ...
    ConfigSaveframe22Position(frame22, "Mainframe22")
    config:delete() -- always delete the config
    event:Skip()
    CloseWatchWindow()
end
frame22:Connect(wx.wxEVT_CLOSE_WINDOW, CloseWindow)

-- ---------------------------------------------------------------------------
-- Finish creating the frame22 and show it
